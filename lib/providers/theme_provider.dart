...
class ThemeProvider with ChangeNotifier {
  static const TTHEME_STATUS =
      "TTHEME_STATUS"; //Tema durumunu key olarak verdik.
  ...
  setDarkTheme({required bool themeValue}) async {
    SharedPreferences prefs = await SharedPreferences
        .getInstance(); //Shared_Preferences paketini indirdik. Ve öyle kullandık.
    prefs.setBool(TTHEME_STATUS, themeValue);
    _darkTheme =
        themeValue; //Eğer getIsDarkTheme durumu neyse ona göre aksiyon almayı sağladık.
  ...
  Future<bool> getTheme() async {
    ...
    _darkTheme = prefs.getBool(TTHEME_STATUS) ??
        false; //Eğer null gelirse false'a eşitle.
    notifyListeners();
    return _darkTheme;
  }
}
